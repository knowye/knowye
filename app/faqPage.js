/*
 * JS for faqPage generated by Appery.io
 */
...
function faqPage_js() {
    ...
    var n2id_buf = {
        'menuButton': 'faqPage_menuButton',
        'feedbackLabel': 'faqPage_feedbackLabel',
        'html_3': 'faqPage_html_3'
    };
    ...
    Apperyio.CurrentScreen = 'faqPage';
     ...
     */
    // On Load
    var faqPage_onLoad = function() {
        faqPage_elementsExtraJS();
        initPage();
        faqPage_deviceEvents();
        faqPage_windowEvents();
        faqPage_elementsEvents();
    };
    // screen window events
    function faqPage_windowEvents() {
        $('#faqPage').bind('pageshow orientationchange', function() {
            var _page = this;
            adjustContentHeightWithPadding(_page);
        });
    };
    // device events
    function faqPage_deviceEvents() {
        document.addEventListener("deviceready", function() {
        });
    };
    // screen elements extra js
    function faqPage_elementsExtraJS() {
        // screen (faqPage) extra code
    };
    // screen elements handler
    function faqPage_elementsEvents() {
        $(document).on("click", "a :input,a a,a fieldset label", function(event) {
            event.stopPropagation();
        });
        $(document).off("click", '#faqPage_faqHeader [name="menuButton"]').on({
            click: function(event) {
                if (!$(this).attr('disabled')) {
                    navigateRB();
                }
            },
        }, '#faqPage_faqHeader [name="menuButton"]');
        $(document).off("click", '#faqPage_faqContainer [name="feedbackLabel"]').on({
            click: function(event) {
                if (!$(this).attr('disabled')) {
        ...
        }, '#faqPage_faqContainer [name="feedbackLabel"]');
    };
    $(document).off("pagebeforeshow", "#faqPage").on("pagebeforeshow", "#faqPage", function(event, ui) {
        Apperyio.CurrentScreen = "faqPage";
        _.chain(Apperyio.mappings)
            .filter(function(m) {
                return m.homeScreen === Apperyio.CurrentScreen;
...
$(document).off("pagecreate", "#faqPage").on("pagecreate", "#faqPage", function(event, ui) {
